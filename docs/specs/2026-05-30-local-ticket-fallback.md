...
| レイヤー | モジュール | 変更内容 |
|---------|-----------|---------|
| Data | data/ticket | 旧 `TicketRepository`（DataStore実装）を `LocalTicketRepository` にリネーム。`ITicketRepository` の公開実装として新たに `TicketRepository` を追加し、DataStoreでTodoistトークン有無を判定して内部で `Local` / `Network` に委譲する。 |
...
| DI | app/di/RepositoriesModule.kt | `ITicketRepository` のバインド先を `NetworkTicketRepository` から `TicketRepository` に変更。 |
...
- [ ] Todoist連携時は `TicketRepository` が `NetworkTicketRepository` に委譲し、従来動作を維持する
- [ ] `LocalTicketRepository` の既存ユニットテスト（旧 `TicketRepositoryTest` をリネームしたもの）が引き続き通る
- [ ] 新たな `TicketRepositoryTest` でトークン有無による分岐が検証される

## 7. テスト方針

| 種別 | 対象 |
|------|------|
| ユニットテスト | 既存 `TicketRepositoryTest`（DataStore実装の検証）を `LocalTicketRepositoryTest` にリネーム追従。新規 `TicketRepository` 用に新しい `TicketRepositoryTest` を作成し、トークン有無で正しいリポジトリに委譲されることを検証する |
| Roborazzi | なし（UI変更なし） |
